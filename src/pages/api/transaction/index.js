...
export default async function handler(req, res) {
  switch (req.method) {
    ...
    case "DELETE":
      return await deleteTransactionByDate(req, res);
    default:
...
const deleteTransactionByDate = async (req, res) => {
  try {
    const { start_date, end_date } = req.query;
    let results = []
    if (start_date != null && start_date !== '' && end_date != null && end_date !== '') {
      results = await pool.query("DELETE FROM transaction WHERE order_date >= ? AND order_date <= ? ", [
        start_date,
        end_date
      ])
    }
